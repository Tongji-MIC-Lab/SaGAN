    ...
    #           # print(torch.cat(output_tensor).size())
        ...
        if do_Dmain or do_Dr1:
            ...
            with torch.autograd.profiler.record_function(name + '_forward'):
                ...
                loss_Dreal_aug = 0
                if do_Dmain:
                    real_img_tmp_aug = real_img.clone().detach().requires_grad_(False)
                    rank = torch.distributed.get_rank()
                    DEVICE = torch.device("cuda",rank)
                    tps = TPS()
                    for i in range(real_img_tmp_aug.shape[0]):
                        ten_img=real_img_tmp_aug[i]
                        source,target=randpoint()
                        ...
                        real_img_tmp_aug[i] = ten_wrp[0]

                    real_logits_aug, real_d_fts_aug = self.run_D(real_img_tmp_aug, real_c, sync=sync, fts=txt_fts_)
                    training_stats.report('Loss/scores/real_aug', real_logits_aug)
                    training_stats.report('Loss/signs/real_aug', real_logits_aug.sign())
                    loss_Dreal_aug = torch.nn.functional.softplus(real_logits_aug) # -log(sigmoid(real_logits))
                    loss_Dreal_aug=torch.cat([loss_Dreal_aug[0:2],loss_Dreal_aug[0:2]],dim=0)
            with torch.autograd.profiler.record_function(name + '_backward'):
                (real_logits * 0 + loss_Dreal + loss_Dr1+loss_Dreal_aug).mean().mul(gain).backward()
