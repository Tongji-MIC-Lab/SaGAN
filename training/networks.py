...
class SynthesisBlock(torch.nn.Module):
    ...
    def forward(self, x, img, ws, fts, styles=None, return_styles=False, force_fp32=False, fused_modconv=None, **layer_kwargs):
...
class DiscriminatorEpilogue(torch.nn.Module):
    def __init__(self,
        ...
        self.prijection=Projection(512)
    ...
    def forward(self, x, img, cmap, use_norm=True, fts=None, force_fp32=False):
        ...
        if self.use_fts:
            ...
            img_pro_fts,txt_pro_fts=self.prijection(d_fts,fts)
            x = x + (img_pro_fts*txt_pro_fts).sum()
        else:
            d_fts = fts
...
class Projection(torch.nn.Module):
    def __init__(self,f_dim):
        super().__init__()
        self.projection1 = FullyConnectedLayer(f_dim, f_dim, activation='lrelu',lr_multiplier=0.01)
        self.projection2 = FullyConnectedLayer(f_dim, f_dim,lr_multiplier=0.01)
    def forward(self, img_fts,txt_fts):
        img_pro_fts=self.projection1(img_fts)
        txt_pro_fts=self.projection1(txt_fts)
        img_pro_fts=self.projection2(img_pro_fts)
        txt_pro_fts=self.projection2(txt_pro_fts)
        return img_pro_fts,txt_pro_fts
